...
{
	Client *l;
	for (l = c->mon->cl->clients; l && l->next; l = l->next);
	if (l) {
		l->next = c;
		c->next = NULL;
	}
...
{
	Client *l;
	for (l = c->mon->cl->stack; l && l->snext; l = l->snext);
	if (l) {
		l->snext = c;
		c->snext = NULL;
	}
...
rotatestack(const Arg *arg)
{
	...
	f = selmon->sel;
	if (arg->i > 0) {
		for (c = nexttiled(selmon->cl->clients, selmon); c && nexttiled(c->next, selmon); c = nexttiled(c->next, selmon));
		if (c){
			detach(c);
			attach(c);
			detachstack(c);
...
solitary(Client *c)
{
	return ((nexttiled(c->mon->cl->clients, c-> mon) == c && !nexttiled(c->next, c->mon))
...
togglescratch(const Arg *arg)
{
	...
	if (found) {
		c->tags = ISVISIBLE(c, c->mon) ? 0 : selmon->tagset[selmon->seltags];
		focus(NULL);
		arrange(selmon);
